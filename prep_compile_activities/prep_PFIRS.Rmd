...
```{r}
# Find the Burn_Unit with 
sf %>% 
  filter(Year == 2021) %>% 
  filter(Acres_Burned == 3) %>% 
  group_by(Year, Acres_Burned, geometry) %>% 
  count() %>% 
  arrange(desc(n))
```

# Find the record with Year == 2021, Acres_Burned == 3, and geometry == (-123.225 39.137)
```{r}
sf %>% 
  filter(Year == 2021) %>% 
  filter(Acres_Burned == 3) %>% 
  filter(geometry == st_sfc(st_point(c(-123.225, 39.137)), crs = 4326)) %>% 
  select(Burn_Date, Burn_Unit, Acres_Burned, Year, geometry)
