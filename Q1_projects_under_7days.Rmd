...
count(CM_size_burntype, any_broadcast_burn, VIIRS_detects) %>% 
  ...
  labs(x = 'Treatment contains broadcast burn', 
      fill = "VIIRS detections")
...
ggplot(CM_size_burntype, aes(as.numeric(trt_acres), as.numeric(VIIRS_detects), 
  ...
  geom_vline(data = data.frame(any_broadcast_burn = c(T, F), 
                               x = c(50, 25)), 
             aes(xintercept = x, color = any_broadcast_burn), lwd = 1, lty = 2) +
