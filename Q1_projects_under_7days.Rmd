...
viirs_int <- st_intersection(viirs_t, CM_7days_trtIDs) %>% 
...
For every treatment polygon, calculate the total duration of burning activity and number of unique days for intersecting VIIRS detections. Compare the two calculations. I also want to look at timing.
...
Duration might look ok, but what about timing? Not great!
...
Look at burn type's influence on detections--slightly more in treatments with broadcast burns than pile burns
```{r}
...
Ok, well is that bc broadcast burns might be over a larger area than pile burns? Not necessarily. If anything, the importance of treatment size seems less for broadcast burns. This figure nonetheless shows that VIIRS generally wont do a good job of detecting activity until >1000 acres. Meanwhile, the median treatment is 97 acres. 

```{r}
# look at size of treatment on probability of any VIIRS detections. 
ggplot(CM_size_burntype, aes(as.numeric(trt_acres), as.numeric(VIIRS_detects), 
                             group = any_broadcast_burn, color = any_broadcast_burn)) +
  geom_jitter(width = 0, height = .02, alpha = .3) +
