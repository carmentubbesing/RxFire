...
### Load
```{r}
frap_dir <- fs::dir_ls(ref_path, recurse = F, glob = '*FRAP*', type = 'directory')
if(length(frap_dir) > 1){
  stop( "There's more than one fire22_1 .gdb directory. script assumes only 1.")
}
```
...
```{r}
act_2020 <- st_difference(frap, NF)
```
...
```{r}
frap_20 <- st_sym_difference(frap_20, NF)
```

```{r}
save(frap_diss_2020, file = "Rdata/frap_2020_dis_Rx.Rdata")
