...
pfirs <- pfirs %>% 
  filter((Burn_Type == "Pile" & Acres_Burned >= 25) | (Burn_Type %in% c("Broadcast", "Unknown/Multiple") & Acres_Burned >= 50))
...
## Sync column names
```{r}
summary(as.factor(pfirs$Agency))
summary(as.factor(cm$Agency_cm))
```
...
cm_merge <- cm %>% 
  ...
  select(-UNIT_OF_MEASURE, -PROJECT_STATUS, -QUANTITY, -ACTIVITY_STATUS, -GROUND_DISTURBING)
```

```{r}
summary(as.factor(cm$ACTIVITY_STATUS))
```
