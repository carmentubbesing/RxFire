...
```{r}
dups <- dups %>% 
  filter(abs(diff_dates_start)<30 | abs(diff_dates_end) < 30) 
```

## View
```{r}
mapview(pfirs_buff_dups, alpha.regions = 0.3)+
  mapview(cm_dups, alpha.regions = 0.3, col.regions = "red") + 
  mapview(dups, alpha.regions = 0.3, col.regions = "yellow")
```
    
```{r}
print(paste("There are ", nrow(dups), "rows that are duplicated."))
print(paste("-  Of those, there are ", length(unique(dups$ID_PFIRS_CT)), "unique PFIRS records"))
print(paste("-  and ", length(unique(dups$ID_CM_Trt)), "unique CalMapper records"))
```

## Write to shapefile

### Rename columns that end up duplicated when truncated
```{r}
sort(names(dups))
```
...
```{r}
st_write(dups_esri, dsn = "shapefiles", layer = "duplicates_final", driver = "ESRI Shapefile", delete_layer = T)
...
## Method 2: Retain information from both
...
mapview(pfirs, zcol = "Year",  alpha.regions = 0.3)+
  ...
  mapview(dups, col.regions = "green", alpha.regions = 0.3)+
  mapview(dups_point, col.regions = "green", alpha.regions = 0.3)
```

## Look at an example where two rows of `dups` have the same PFIRS ID
```{r}
dups %>% 
  filter(duplicated(ID_PFIRS_CT)) %>% 
  arrange(ID_PFIRS_CT) 
```
